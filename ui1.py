...
class FacturationApp(tk.Tk):
    ...
    def _build_sidebar(self, parent):
        ...
        actions = [
            ...
            ("🖨  Imprimer",             self._imprimer,          "#0891b2"),
    ...
    def _build_quick_entry(self, parent):
        ...
        self.entry_quick_ref.bind("<Escape>",     lambda e: self._quick_popup_close())
    ...
    def _quick_ref_key(self, event):
        """KeyRelease dans le champ Référence : déclenche la recherche textuelle."""
        # Ignorer les touches de navigation
        if event.keysym in ("Return", "Down", "Up", "Escape", "Tab"):
            return
        self.lbl_quick_status.config(text="", fg="#059669")
        # Vider les champs si l'utilisateur re-tape
        self.var_quick_desig.set("")
        self.var_quick_montant.set("—")
        self._quick_prix_unitaire = 0.0
        self._quick_search_live()
    ...
    def _quick_search_live(self):
        """Recherche en direct : code-barres exact, référence exacte, puis textuelle."""
        ...
        if produit:
            ...
            self._quick_popup_close()
            return
        # 3. Recherche textuelle → dropdown
        results = db.search_catalogue(query)
        if len(results) == 1:
            self._quick_fill(results[0])
            self._quick_popup_close()
        elif len(results) > 1:
            self._quick_show_popup(results[:12])
        else:
            self._quick_popup_close()
    ...
    def _quick_fill(self, produit: dict):
        ...
        self._quick_popup_close()
        # Le curseur reste dans le champ référence — pas de focus sur la quantité
        # (évite le débordement du code-barres de la douchette dans la quantité)
    ...
    def _quick_show_popup(self, produits: list):
        """Affiche une liste déroulante de suggestions sous le champ Référence."""
        self._quick_popup_close()

        popup = tk.Toplevel(self)
        popup.overrideredirect(True)  # Sans décoration
        popup.configure(bg=COLORS["border"])

        # Position sous le champ référence
        self.update_idletasks()
        x = self.entry_quick_ref.winfo_rootx()
        y = self.entry_quick_ref.winfo_rooty() + self.entry_quick_ref.winfo_height() + 2
        w = max(360, self.entry_quick_ref.winfo_width() * 2)
        h = min(len(produits) * 28 + 4, 280)
        popup.geometry(f"{w}x{h}+{x}+{y}")
        popup.lift()

        self._quick_popup = popup
        self._quick_popup_items = produits
        ...
        for p in produits:
            ...
            ref_txt = f"[{cb}]  " if cb else ""
            lb.insert("end", f"  {ref_txt}{p['designation']}   —   {self._fmt_price(p['prix_unitaire'])} FCFA")
        ...
        def on_select(event=None):
            sel = lb.curselection()
            if sel:
                self._quick_popup_select(sel[0])
        ...
        lb.bind("<Escape>",          on_key)

        # Fermer si on clique ailleurs
        popup.bind("<FocusOut>", lambda e: self.after(150, self._check_popup_focus))

    def _check_popup_focus(self):
        """Ferme le popup si le focus a quitté la fenêtre popup."""
        if self._quick_popup and self._quick_popup.winfo_exists():
            try:
                focused = self.focus_get()
                if focused and str(focused).startswith(str(self._quick_popup)):
                    return
            except Exception:
                pass
            self._quick_popup_close()
    ...
    def _quick_popup_focus(self, event=None):
        """Déplace le focus dans le listbox dropdown."""
        if self._quick_popup and self._quick_popup.winfo_exists():
            self._quick_lb.focus()
            if self._quick_lb.size() > 0:
                self._quick_lb.selection_set(0)
                self._quick_lb.activate(0)

    def _quick_popup_select(self, index: int):
        """Sélectionne un élément du dropdown."""
        if 0 <= index < len(self._quick_popup_items):
            self._quick_fill(self._quick_popup_items[index])

    def _quick_popup_close(self):
        """Ferme et détruit le popup de suggestions."""
